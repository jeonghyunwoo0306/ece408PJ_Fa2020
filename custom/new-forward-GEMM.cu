...
// Simple shared Matmul kernel 
// JH: Remove output reshape kernel and do when we write the data in
// MatrixMultiply kernel
__global__ void matrixMultiplyShared(float *B, float *C,
    int numARows, int numAColumns,
    int numBRows, int numBColumns,
    int numCRows, int numCColumns, int base, const int W_out) {
    ...
    if((Row < numCRows) && (Col < numCColumns))
        // C[Row*numCColumns+Col] = Pvalue;
        C[base + Row*numCColumns + (Col/W_out)*W_out + (Col%W_out)] = Pvalue;
    }
}
...
__host__ void GPUInterface::conv_forward_gpu(float *host_y, const float *host_x, const float *host_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    // JH: Comment out to remove output reshpae kernel
    ...
    // JH: Comment out to remove output reshape kernel
    ...
    for(int i = 0; i < B; i++) {
        ...
        // matrixMultiplyShared<<<DimGridMatmul, DimBlockMatmul>>>(device_k, device_unroll_x, device_unroll_y, M, (K * K * C), (K * K * C), (W_out * H_out), (M), (W_out*H_out));
        

        // matrixMultiplyShared<<<DimGridMatmul, DimBlockMatmul>>>(device_unroll_x, device_unroll_y, M, (K * K * C), (K * K * C), (W_out * H_out), (M), (W_out*H_out));
        //TODO: Erase above kernels (2) after integrate output reshpae kernel into Matmutl kernel
        matrixMultiplyShared<<<DimGridMatmul, DimBlockMatmul>>>(device_unroll_x, device_y, M, (K * K * C), (K * K * C), (W_out * H_out), (M), (W_out*H_out), baseY, W_out);


        // Launch kernel which reshape output from Matmul to orignal shape
        // TODO: Erase this kernel after integrate output reshpae kernel into MatMul kernel
