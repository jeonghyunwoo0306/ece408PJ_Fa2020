...
__global__ void matrixMultiplyShared(float *B, float *C,
    ...
    int numCRows, int numCColumns, int baseX, int baseY, const int W_out, const int W, const int H) {
    ...
    if((temp_row < numBRows) && (Col < numBColumns))
        subTileB[ty][tx] = B[baseX + input_c * (W * H) + (input_h + input_p) * W + (input_w + input_q)];
    else
        subTileB[ty][tx] = 0.0;
    ...
    if((Row < numCRows) && (Col < numCColumns))
        C[baseY + Row*numCColumns + (Col/W_out)*W_out + (Col%W_out)] = Pvalue;
    }
}
...
__host__ void GPUInterface::conv_forward_gpu(float *host_y, const float *host_x, const float *host_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    // TODO: Change gridDim.z to # of Batchs
    ...
    for(int i = 0; i < B; i++) {
        ...
        matrixMultiplyShared<<<DimGridMatmul, DimBlockMatmul>>>(device_x, device_y, M, (K * K * C), (K * K * C), (W_out * H_out), (M), (W_out*H_out), baseX, baseY, W_out, W, H);
    }
